...
class BootstrapServerConnection:
    ...
    def connect_to_bs(self):
        ...
        Args:
            bs (CMS.Node): Bootstrap server node
            me (CMS.Node): This node
        Returns:
            list(CMS.Node) : List of other nodes in the distributed system
        Raises:
        ...
        if (num == 0):
            ...
            return [CMS.Node(toks[3], int(toks[4]), toks[5])]
        else:
            l = range(1, num+1)
            shuffle(l)
            return [CMS.Node(toks[l[0]*3], int(toks[l[0]*3+1]), toks[l[0]*3+2]), CMS.Node(toks[l[1]*3], int(toks[l[1]*3+1]), toks[l[1]*3+2])]
